...
type CreateGraphDefsPayload struct {
	...
	Metrics     []CreateGraphDefsPayloadMetrics `json:"metrics"`
}

type CreateGraphDefsPayloadMetrics struct {
	Name        string `json:"name"`
	DisplayName string `json:"displayName"`
...
func (api *API) CreateGraphDefs(payloads []CreateGraphDefsPayload) error {
	...
	logger.Debugf("Create graph defs request: %s", string(requestJSON))
